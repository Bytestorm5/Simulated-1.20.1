...
public class OffroadDatagen {
    ...
    public static void gatherDataHighPriority(final GatherDataEvent event) {
        if (isGeneratingFor(event)) {
            OffroadTags.addGenerators();
        }
    }

    /**
     * 1.20.1: GatherDataEvent is posted on every mod's bus; this checks that data is being generated for Offroad.
     */
    public static boolean isGeneratingFor(final GatherDataEvent event) {
        return event.getModContainer().getModId().equals(Offroad.MOD_ID);
    }
    ...
    public static void gatherData(final GatherDataEvent event) {
        ...
        generator.addProvider(event.includeClient(), OffroadSoundEvents.REGISTRY.getProvider(output));
    }
